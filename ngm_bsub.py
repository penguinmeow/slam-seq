...
def create_job_file_pe(reference, samplefile1, samplefile2, in_dir, out_dir):
    ...
    job_body1 = job_body1.format(reference, samplefile1, samplefile2, out_dir + '/' + basename+'.bam')
    jobfile = prefix + ".sh"
...
for sample in range(0,len(fastqR1)):
    submit_job(create_job_file_pe(args.reference, fastqR1[sample], fastqR2[sample], args.input_dir, args.out_dir))
